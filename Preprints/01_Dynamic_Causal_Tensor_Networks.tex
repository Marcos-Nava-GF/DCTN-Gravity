...
\begin{table}[h!]
    ...
    \begin{tabular}{cll}
        ...
        \midrule
        $\gamma$ & Causal Cost Exponent (Topology) & $\gamma_c \approx 2.5$ (Phase Transition) \\
    ...
    \footnotesize
    \vspace{0.1cm}
    \\ \textit{Note: The values presented are results derived from stochastic simulations described throughout the preprint trilogy. $\gamma$ replaces the symbol $\alpha$ used in previous versions to avoid confusion with the Fine-Structure Constant.}
\end{table}
...
The evolution of the network is governed by a connection probability $P$ that competes between cohesion ($\beta$) and causal separation ($\gamma$):
\begin{equation}
P(T_{\text{new}} | \{ T_i \}) \propto \frac{(\sum \text{deg}_i)^\beta}{[\Delta \tau_{ij}]^\gamma}
\end{equation}

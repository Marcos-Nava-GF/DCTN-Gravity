...
def analyze_topology(G):
    ...
    # 1. Ricci Curvature (Ollivier-Ricci)
    # Measures the 'roundness' or 'gravity' of the network locally
    print("Calculating Ollivier-Ricci Curvature (this may take time)...")
    ...
    # Sample nodes to speed up calculation
    sample_size = min(50, len(G.nodes()))
    seeds = np.random.choice(G_ricci.nodes(), size=sample_size, replace=False)
...
def visualize_results(G_ricci, slope, r_val, radii_list, mass_list):
    ...
    # edge_curvatures = [d['ricciCurvature'] for u, v, d in G_ricci.edges(data=True)] # Opional
    node_curvatures = [np.mean([G_ricci[u][v]['ricciCurvature'] for v in G_ricci[u]]) for u in G_ricci.nodes()]

    avg_curv = np.mean(node_curvatures)

    # --- COLOR FIX: Mapeo Manual ---
    # Calculamos los colores RGBA explícitamente para evitar el error de 'norm'
    # Esto fuerza a que el 0 sea blanco, ROJO sea positivo, AZUL sea negativo
    norm = mcolors.TwoSlopeNorm(vmin=min(node_curvatures), vcenter=0, vmax=max(node_curvatures))
    cmap = plt.cm.coolwarm

    # Creamos un objeto "Mappable" y generamos la lista de colores
    sm = plt.cm.ScalarMappable(norm=norm, cmap=cmap)
    node_colors_explicit = [sm.to_rgba(c) for c in node_curvatures]

    # Dibujamos pasando la lista de colores exacta (sin argumentos 'norm' ni 'cmap')
